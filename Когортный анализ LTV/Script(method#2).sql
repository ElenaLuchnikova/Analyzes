with t1 as (
  ...
  case when max(diff)> 0 then round(sum(case when diff<=14 then sale end)/count(distinct user),2) end as "14",
  case when max(diff)> 0 then round(sum(case when diff<=30 then sale end)/count(distinct user),2) end as "30",
